...
interface ResponsiveCardProps<T = unknown> {
  cardData: {
    ...
    items: {
      itemLabel: string;
      itemInfo: string | number | ReactNode;
    }[];
  };
  rawData: T;
...
export function ResponsiveCard<T = unknown>({
  handleToggleDialog,
  cardData: {
    id,
    title,
  ...
  return (
    <div className="space-y-4 md:hidden">
      <Card
        ...
        onClick={ () => handleToggleDialog(true, rawData) }
      >
          ...
          {transferableIndicator && (
            <div
              className={ `flex items-center gap-1 ${
                transferable ? "text-green-500" : "text-red-500"
              }` }
            >
              {transferable ? (
                <>
