...
import { getNewClientsMetric } from "@/services/dashboard.service";
...
export function NewClientsCard({
  ...
  useEffect(() => {
    async function fetchMetric() {
      ...
      setLoading(true);
      try {
        const { data: metricData } = await getNewClientsMetric({
        ...
        console.error("Failed to fetch new clients metric", error);
        setData([]);
      } finally {
        setLoading(false);
      }
    ...
    <Card className="col-span-1 md:col-span-2">
      <CardHeader>
        ...
        <CardDescription className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <span>Evolução de novos clientes por mês</span>
